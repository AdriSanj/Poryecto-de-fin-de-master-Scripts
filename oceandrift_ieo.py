...
#from opendrift.models.larvalfish_ieo import *
...
class OceanDrift(OpenDriftSimulation):
    ...
    #def update_terminal_velocity(self, Tprofiles=None, Sprofiles=None,
                                 #z_index=None, DENSsp = None):					#### AQUI CAMBIE LO DEL TIME_INDEX
    ...
    def get_diffusivity_profile(self, model):
        ...
        MLD = self.environment.ocean_mixed_layer_thickness  # TODO: obtain from readers/environment
    ...
    def vertical_mixing(self, store_depths=False):
        ...
        #print('longitudes 370')
        #print(len(Kprofiles))
        #print(Kprofiles)
        #print(len(self.environment_profiles['z']))
        #print(self.environment_profiles['z'])
        gradK = -np.gradient(Kprofiles, self.environment_profiles['z'], axis=0)
        gradK[np.abs(gradK)<1e-10] = 0
        #f = open('data.txt', 'w')      # Para comprobar que lee bien las densidades iniciales
        ...
        for i in range(0, ntimes_mix):
            ...
            if i == 0:			#### AQUI ES NUEVO
                DENSsp = self.first_DENSsp(Tprofiles=Tprofiles, Sprofiles=Sprofiles, z_index=z_index)
            #print(DENSsp)
            #f.writelines(str(DENSsp))
            # Update the terminal velocity of particles
            #print('entrada')
            #print(DENSsp[0])
            
            w = np.zeros(len(self.elements.hatched))
            eggs = np.where(self.elements.hatched == 0)[0]
            
            if len(eggs) != 0:
                self.update_terminal_velocity(DENSsp=DENSsp)		
                w[eggs] = self.elements.terminal_velocity[eggs]
            print(w)
            #w = 0.
            ...
            #print('ASDFGSFBGDH')
            #print(len(self.elements.z))
            #print(len(w))
            #print(len(self.elements.moving))
            
            
            if len(eggs) != 0:
                self.elements.z[eggs] = self.elements.z[eggs] + w[eggs]*dt_mix*self.elements.moving[eggs]
    ...
    def first_DENSsp(self,Tprofiles=None, Sprofiles=None, z_index=None):
        ...
        #print('Densidad del agua')
        #print(DENSw[0])
        DENSsp = 0.78797*(DENSw-1000.) + 4.2519
      #DENSsp=2000.
        return DENSsp+1000.
